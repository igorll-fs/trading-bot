...
class BotLearningSystem:
    ...
    def __init__(self, db):
        ...
        self.min_trades_for_adjustment = int(
        ...
        self.param_bounds = {
            "min_confidence_score": (0.40, 0.70),  # 40-70% (alinhado com guardrails)
            "stop_loss_multiplier": (0.70, 1.20),  # 0.7x-1.2x (alinhado com guardrails)
            "take_profit_multiplier": (0.60, 1.50),  # 0.6x-1.5x (alinhado com guardrails)
            "position_size_multiplier": (0.50, 1.30),  # 0.5x-1.3x (alinhado com guardrails)
        }
    ...
    async def initialize(self):
        ...
        try:
            ...
            learned_params = await self.db.learning_data.find_one(
                {"type": "parameters"}, sort=[("timestamp", -1)]
            )
            ...
            if learned_params:
                ...
                logger.info(
                    f"Total de trades anteriores: {self.performance_metrics.get('total_trades', 0)}"
                )
                logger.info(
                    f"Win Rate historico: {self.performance_metrics.get('win_rate', 0):.1f}%"
                )
                logger.info(
                    f"Confidence Score Minimo: {self.adjustable_params['min_confidence_score']:.2f}"
                )
                logger.info(
                    f"Stop Loss Multiplier: {self.adjustable_params['stop_loss_multiplier']:.2f}x"
                )
                logger.info(
                    f"Take Profit Multiplier: {self.adjustable_params['take_profit_multiplier']:.2f}x"
                )
                logger.info(
                    f"Position Size Multiplier: {self.adjustable_params['position_size_multiplier']:.2f}x"
                )
            else:
                logger.info(
                    "Nenhum aprendizado anterior encontrado. Iniciando com parametros padrao."
                )
    ...
    async def _update_performance_metrics(self):
        ...
        try:
            ...
            trades = (
                await self.db.trades.find({}, {"pnl": 1, "_id": 0})
                .sort("timestamp", -1)
                .limit(1000)
                .to_list(1000)
            )
            ...
            # Calcular medias
            if winning:
                self.performance_metrics["avg_profit"] = statistics.mean(
            ...
            logger.info(
                f"Metricas atualizadas: {self.performance_metrics['total_trades']} trades, Win Rate: {self.performance_metrics['win_rate']:.1f}%"
            )
    ...
    def _update_param(self, name: str, new_value: float, reason: str):
        ...
        # 🛡️ ML GUARDRAILS: Validar ANTES de qualquer processamento
        is_safe_target, warning = self._validate_safety(name, new_value)
        if not is_safe_target:
            logger.error(
                "🚫 GUARDRAIL BLOQUEOU TARGET: %s=%.3f -> %.3f REJEITADO - %s",
                name,
                current,
                new_value,
                warning,
            )
            return current  # Rejeitar imediatamente
        ...
        if abs(clamped - current) > 0.001:  # Só atualiza se houver mudança significativa
            # 🛡️ Segunda checagem de segurança no valor suavizado final
            is_safe_final, warning_final = self._validate_safety(name, clamped)
            if not is_safe_final:
                logger.error(
                    "🚫 GUARDRAIL BLOQUEOU FINAL: %s de %.3f para %.3f - MOTIVO: %s",
                    name,
                    current,
                    clamped,
                    warning_final,
                )
                return current  # Rejeitar mudança perigosa
    ...
    def _validate_safety(self, param_name: str, proposed_value: float) -> tuple[bool, str]:
        """
        🛡️ ML GUARDRAILS: Validação de segurança para prevenir parâmetros suicidas.

        Retorna: (is_safe: bool, warning_message: str)

        HARD LIMITS (IMUTÁVEIS):
        - min_confidence_score: 0.40 - 0.70 (nunca < 40% = suicídio, nunca > 70% = paralisia)
        - stop_loss_multiplier: 0.70 - 1.20 (nunca < 0.7 = SL muito apertado, nunca > 1.2 = perda excessiva)
        - take_profit_multiplier: 0.60 - 1.50 (nunca < 0.6 = lucro mínimo, nunca > 1.5 = hold excessivo)
        - position_size_multiplier: 0.50 - 1.30 (nunca < 0.5 = ineficiência, nunca > 1.3 = overexposure)

        Filosofia: "Preservação de capital > Lucro especulativo"
        """
        SAFETY_LIMITS = {
            "min_confidence_score": {
                "min": 0.40,  # 40% - Menos que isso = entrar em lixo
                "max": 0.70,  # 70% - Mais que isso = paralisia por análise
                "danger_min": 0.35,  # 35% - Zona de perigo crítico
                "danger_max": 0.75,  # 75% - Zona de paralisia
            },
            "stop_loss_multiplier": {
                "min": 0.70,  # 0.7x - SL muito apertado = stops prematuros
                "max": 1.20,  # 1.2x - SL muito largo = perdas inaceitáveis
                "danger_min": 0.50,  # 0.5x - CRÍTICO: SL absurdamente apertado
                "danger_max": 1.50,  # 1.5x - CRÍTICO: Perda > 3% no E7450 é suicídio
            },
            "take_profit_multiplier": {
                "min": 0.60,  # 0.6x - TP muito próximo = lucros insignificantes
                "max": 1.50,  # 1.5x - TP muito distante = holding excessivo
                "danger_min": 0.40,  # 0.4x - CRÍTICO: Quase não lucra
                "danger_max": 2.00,  # 2.0x - CRÍTICO: TP irreal
            },
            "position_size_multiplier": {
                "min": 0.50,  # 0.5x - Muito conservador (mas ok)
                "max": 1.30,  # 1.3x - MAX overexposure tolerável
                "danger_min": 0.30,  # 0.3x - CRÍTICO: Trades ineficientes
                "danger_max": 1.80,  # 1.8x - CRÍTICO: Overexposure suicida
            },
        }

        limits = SAFETY_LIMITS.get(param_name)
        if not limits:
            # Parâmetro não monitorado - permitir
            return True, "Parameter not monitored by guardrails"

        # VERIFICAÇÃO 1: Zona de perigo crítico (danger zone)
        if proposed_value <= limits["danger_min"]:
            return (
                False,
                f"CRÍTICO: {proposed_value:.3f} <= {limits['danger_min']:.3f} (danger zone min). REJEITAR IMEDIATAMENTE.",
            )

        if proposed_value >= limits["danger_max"]:
            return (
                False,
                f"CRÍTICO: {proposed_value:.3f} >= {limits['danger_max']:.3f} (danger zone max). REJEITAR IMEDIATAMENTE.",
            )

        # VERIFICAÇÃO 2: Limites normais de segurança
        if proposed_value < limits["min"]:
            return (
                False,
                f"UNSAFE: {proposed_value:.3f} < {limits['min']:.3f} (safe min). Valor muito baixo.",
            )

        if proposed_value > limits["max"]:
            return (
                False,
                f"UNSAFE: {proposed_value:.3f} > {limits['max']:.3f} (safe max). Valor muito alto.",
            )
    ...
    def adjust_stop_loss(self, base_stop_loss: float, entry_price: float = None) -> float:
        ...
        logger.debug(
            f"SL ajustado: {base_stop_loss:.4f} -> {adjusted:.4f} (dist: {original_distance:.4f} -> {adjusted_distance:.4f}, mult: {multiplier:.2f}x)"
        )
    ...
    def adjust_take_profit(self, base_take_profit: float, entry_price: float = None) -> float:
        ...
        logger.debug(
            f"TP ajustado: {base_take_profit:.4f} -> {adjusted:.4f} (dist: {original_distance:.4f} -> {adjusted_distance:.4f}, mult: {multiplier:.2f}x)"
        )
        return round(adjusted, 4)
    ...
    def adjust_position_size(self, base_position_size: float) -> float:
        ...
        logger.debug(
            f"Position size ajustado: {base_position_size:.2f} -> {adjusted:.2f} (mult: {multiplier:.2f}x)"
        )
        return adjusted
    ...
    async def learn_from_trade(self, trade: Dict):
        ...
        try:
            ...
            if target_confidence != current_confidence:
                self._update_param(
                    "min_confidence_score", target_confidence, f"Win Rate {win_rate:.1f}%"
                )
            ...
            if pnl < 0:  # Trade perdedor
                ...
                if avg_loss > 50:  # Perdas muito grandes - apertar SL
                    self._update_param(
                        "stop_loss_multiplier", current_sl_mult - 0.05, "Perdas grandes detectadas"
                    )
    ...
    async def _save_trade_analysis(self, trade: Dict):
        ...
        try:
            ...
            logger.debug(
                f"Análise salva: {trade.get('symbol')} - Won={pnl > 0} - ML Score={trade.get('confidence_score', 0.0):.2f}"
            )
    ...
    async def get_learning_stats(self) -> Dict:
        ...
        return {
            ...
            "total_learning_records": await self.db.learning_data.count_documents(
                {"type": "bot_parameters"}
            ),
            "total_analyses": await self.db.learning_data.count_documents(
                {"type": "trade_analysis"}
            ),
        }
