...
class AdvancedLearningSystem:
    ...
    def __init__(self, db):
        ...
        # Configurações
        self.min_trades_for_learning = int(
            os.getenv("LEARNING_MIN_TRADES", "15")
        )  # Threshold reduzido para aprendizado mais rápido
        logger.info(
            f"[ML] Trade threshold set to {self.min_trades_for_learning} trades before optimization"
        )
        self.confidence_interval = float(os.getenv("LEARNING_CONFIDENCE", "0.95"))
        self.max_param_change = float(
            os.getenv("LEARNING_MAX_CHANGE", "0.10")
        )  # Max 10% por ajuste
    ...
    async def _calculate_advanced_metrics(self):
        ...
        self.metrics["expectancy"] = (
            win_rate * self.metrics["avg_win"] + (1 - win_rate) * self.metrics["avg_loss"]
        )
    ...
    async def learn_from_trade(self, trade: dict):
        ...
        try:
            ...
            logger.info(
                f"[ML] Métricas: WR={self.metrics['win_rate']:.1f}% | PF={self.metrics['profit_factor']:.2f} | Exp=${self.metrics['expectancy']:.2f}"
            )

            # Só ajusta parâmetros se tiver trades suficientes
            if self.metrics["total_trades"] < self.min_trades_for_learning:
                logger.info(
                    f"[ML] Coletando dados: {self.metrics['total_trades']}/{self.min_trades_for_learning}"
                )
    ...
    def calculate_opportunity_score(self, opportunity: dict, market_conditions: dict) -> float:
        ...
        try:
            ...
            if (trend == "bullish" and signal == "BUY") or (
                trend == "bearish" and signal == "SELL"
            ):
                score += 0.1  # Tendência alinhada com sinal
            elif (trend == "bullish" and signal == "SELL") or (
                trend == "bearish" and signal == "BUY"
            ):
    ...
    # ------------------------------------------------------------------
    # Métodos de ajuste de parâmetros (compatibilidade com trading_bot.py)
    ...
    def adjust_stop_loss(self, base_stop_loss: float, entry_price: float | None = None) -> float:
        ...
        - Recomputa pelo % aprendido para manter consistência de unidades.
        """
        if not self.learning_enabled or entry_price is None:
            return base_stop_loss
        try:
            sl_pct = self.params["stop_loss_percent"] / 100  # ex: 2.0% → 0.02
            if base_stop_loss < entry_price:
                # BUY — SL abaixo do entry
                adjusted = round(entry_price * (1 - sl_pct), 4)
            else:
                # SELL — SL acima do entry
                adjusted = round(entry_price * (1 + sl_pct), 4)
            logger.debug(
                "SL ajustado (ML): %.4f -> %.4f (%.2f%%)",
                base_stop_loss,
                adjusted,
    ...
    def adjust_take_profit(self, base_take_profit: float, entry_price: float | None = None) -> float:
        ...
        try:
            tp_pct = self.params["take_profit_percent"] / 100  # ex: 3.0% → 0.03
            if base_take_profit > entry_price:
                # BUY — TP acima do entry
                adjusted = round(entry_price * (1 + tp_pct), 4)
            else:
                # SELL — TP abaixo do entry
                adjusted = round(entry_price * (1 - tp_pct), 4)
            logger.debug(
                "TP ajustado (ML): %.4f -> %.4f (%.2f%%)",
                base_take_profit,
                adjusted,
                self.params["take_profit_percent"],
            )
            return adjusted
        except Exception as e:
            logger.warning("adjust_take_profit erro: %s", e)
            return base_take_profit

    def adjust_position_size(self, base_position_size: float) -> float:
        """Ajusta tamanho da posição pelo % aprendido (position_size_percent)."""
        if not self.learning_enabled:
            return base_position_size
        try:
            # position_size_percent representa % do capital; usamos como escalar relativo ao valor base
            # Mantém proporção: se sistema aprendeu 6% vs padrão 5%, multiplica por 1.2
            default_pct = 5.0  # Alinhado com BotConfig padrão
            multiplier = self.params["position_size_percent"] / default_pct
            adjusted = base_position_size * multiplier
            logger.debug(
                "Position size ajustado (ML): %.2f -> %.2f (x%.2f)",
                base_position_size,
                adjusted,
                multiplier,
            )
