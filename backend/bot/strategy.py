...
class TradingStrategy:
    ...
    # Tamanho máximo buscado da API — todas as chamadas compartilham o mesmo cache
    # e recebem df.tail(limit) de volta, eliminando cache misses por limit diferente.
    ...
    def get_historical_data(
        ...
        menor (ex: limit=20 para LLM, limit=30 para ATR) reutilizam a mesma
        entrada de cache e recebem df.tail(limit), evitando chamadas duplicadas à API.
        """
        try:
            timeframe = timeframe or self.timeframe
            requested_limit = limit or self.limit
            # Chave de cache sem limit — sempre usamos o tamanho máximo
            cache_key = f"klines_{symbol}_{timeframe}"
            cached_data = self.cache.get(cache_key)

            if cached_data is not None:
                logger.debug("Cache HIT para %s", symbol)
                return cached_data.tail(requested_limit).reset_index(drop=True)
            ...
            # Armazenar o DataFrame completo no cache (TTL: 5 segundos)
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if data_len < 26:  # Mínimo para EMA slow
                logger.warning(
                    "Dados insuficientes para calcular indicadores: %d linhas (minimo 26)", data_len
                )
                return df
            ...
            try:
                if "taker_buy_quote" in df.columns and "quote_volume" in df.columns:
                    ...
                    quote_vol = pd.to_numeric(df["quote_volume"], errors="coerce").replace(
                        0, np.nan
                    )
    ...
    def calculate_btc_correlation(self, symbol: str, lookback: int = 30) -> float:
        ...
        Resultado cacheado por 5 minutos — correlação muda lentamente e o
        cálculo de pct_change+corr é desnecessário a cada abertura de posição.
        ...
        try:
            ...
            if cached is not None and (now - cached[1]) < self._BTC_CORRELATION_TTL:
                return cached[0]
            ...
            result = float(correlation) if not pd.isna(correlation) else 0.5

            self._btc_correlation_cache[symbol] = (result, now)
            return result
            ...
            if second_half_low < first_half_low * 0.998:  # Tolerância 0.2%
                if second_half_rsi_at_low > first_half_rsi_at_low * 1.02:  # RSI 2% maior
                    logger.debug(
                        ...
                        first_half_low,
                        second_half_low,
                        first_half_rsi_at_low,
                        second_half_rsi_at_low,
                    )
                    return "bullish"
            ...
            if second_half_high > first_half_high * 1.002:  # Tolerância 0.2%
                if second_half_rsi_at_high < first_half_rsi_at_high * 0.98:  # RSI 2% menor
                    logger.debug(
                        ...
                        first_half_high,
                        second_half_high,
                        first_half_rsi_at_high,
                        second_half_rsi_at_high,
                    )
                    return "bearish"
    ...
    def _get_adaptive_atr_multipliers(self, df: pd.DataFrame, current_atr: float) -> tuple:
        ...
        try:
            ...
            if volatility_ratio > 1.5:
                ...
                logger.debug(
                    "Volatilidade ALTA (ratio=%.2f) - usando ATR mult 2.5/3.0", volatility_ratio
                )
                return 2.5, 3.0
            elif volatility_ratio > 1.2:
                # Volatilidade acima da média
                logger.debug(
                    "Volatilidade MÉDIA-ALTA (ratio=%.2f) - usando ATR mult 2.0/2.5",
                    volatility_ratio,
                )
                return 2.0, 2.5
                ...
                logger.debug(
                    "Volatilidade BAIXA (ratio=%.2f) - usando ATR mult 1.2/1.5", volatility_ratio
                )
                return 1.2, 1.5
            elif volatility_ratio < 0.9:
                # Volatilidade abaixo da média
                logger.debug(
                    "Volatilidade MÉDIA-BAIXA (ratio=%.2f) - usando ATR mult 1.5/1.8",
                    volatility_ratio,
                )
                return 1.5, 1.8
            else:
        ...
        try:
            ...
            if not pd.isna(buy_vol_pct):
                if buy_vol_pct > 0.58:  # CORREÇÃO: Aumentado de 55% para 58%
                    buy_score += 1.5
                    logger.debug(
                        "Volume de compra dominante (%.1f%%) - bonus para BUY", buy_vol_pct * 100
                    )
                elif (
                    buy_vol_pct < 0.52
                ):  # CORREÇÃO: Penalizar se não houver pressão compradora (era <0.45)
                    buy_score -= 2.0  # CORREÇÃO: Aumentado de -1.5 para -2.0
                    logger.debug(
                        "Pressão compradora fraca (%.1f%%) - penalidade aplicada", buy_vol_pct * 100
                    )
                elif buy_vol_pct < 0.45:  # Vendedores dominando muito (<45%)
                    sell_score += 1.0
                    logger.debug(
                        "Volume de venda dominante (%.1f%%) - bonus para SELL", buy_vol_pct * 100
                    )
            ...
            if not pd.isna(current_adx) and current_adx < 25:
                ...
                sell_score >= activation_threshold and higher_trend == "bearish"
            ):  # Must be bearish
                signal = "SELL"
            ...
            if stop_loss is not None and take_profit is not None:
                risk = (
                    latest["close"] - stop_loss if signal == "BUY" else stop_loss - latest["close"]
                )
                reward = (
                    take_profit - latest["close"]
                    if signal == "BUY"
                    else latest["close"] - take_profit
                )
                if risk > 0:
    ...
    def analyze_symbol(self, symbol: str) -> dict | None:
        ...
        try:
            ...
            higher_df = self.get_historical_data(
                symbol, timeframe=self.confirmation_timeframe, limit=300
            )
