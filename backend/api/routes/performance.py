...
async def _build_performance_snapshot(db) -> dict[str, Any]:
    ...
    async with lock:
        ...
        if (
            _performance_cache["data"] is not None
            and (now - _performance_cache["ts"]) < _performance_cache_ttl
        ):
            return _performance_cache["data"]

        # Filtrar apenas trades reais (excluir simulados)
        trades = (
            await db.trades.find({"simulated": {"$ne": True}}, {"_id": 0})
            .sort("closed_at", 1)
            .to_list(1000)
        )
        ...
        initial_balance = float(
            os.environ.get("INITIAL_BALANCE_USDT", "1000")
    ...
    async def get_sparkline(points: int = 50):
        ...
        try:
            ...
            if (
                ...
                and (now - _sparkline_cache["ts"]) < _sparkline_cache_ttl
            ):
            ...
            for trade in reversed(trades):
                ...
                sparkline_data.append(
                    {
                        "timestamp": (
                            trade.get("closed_at", "").isoformat()
                            if hasattr(trade.get("closed_at", ""), "isoformat")
                            else str(trade.get("closed_at", ""))
                        ),
    ...
    async def get_realtime_stats():
        ...
        try:
            ...
            if (
                ...
                and (now - _realtime_cache["ts"]) < _realtime_cache_ttl
            ):
            ...
            try:
                ...
                if bot and hasattr(bot, "client") and bot.client:
                    await asyncio.wait_for(
                        asyncio.get_event_loop().run_in_executor(None, bot.client.ping), timeout=5.0
                    )
